...
void configfile_save(const char *filename) {
    ...
    for (unsigned int i = 0; i < ARRAY_LEN(options); i++) {
        ...
        switch (option->type) {
            case CONFIG_TYPE_BOOL:
                length += sprintf(file + length, "%s %s\n", option->name, *option->boolValue ? "true" : "false");
                break;
            case CONFIG_TYPE_UINT:
                length += sprintf(file + length, "%s %u\n", option->name, *option->uintValue);
                break;
            case CONFIG_TYPE_FLOAT:
                length += sprintf(file + length, "%s %f\n", option->name, *option->floatValue);
                break;
            case CONFIG_TYPE_BIND:
                length += sprintf(file + length, "%s ", option->name);
                for (int i = 0; i < MAX_BINDS; ++i)
                    length += sprintf(file + length, "%04x ", option->uintValue[i]);
                length += sprintf(file + length, "\n");
                break;
            default:
